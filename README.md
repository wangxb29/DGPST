...
For different inputs, please try adjusting the ```controlnet_conditioning_scale``` parameter in line 331 of ```./models/DGPST_model.py```. Additionally, consider replacing ```pil_image=real_B_w``` with ```pil_image=real_B_img``` in this line to better transfer reference styles for ​​cross-domain style transfer​​ scenarios.
