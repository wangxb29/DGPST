...
from diffusers.schedulers import DDIMInverseScheduler, DDIMScheduler, KarrasDiffusionSchedulers
...
def inversion_step(
    ...
    for i in range(num_renoise_steps + 1):
        ...
        with torch.no_grad():
            ...
            noise_pred = unet_pass(pipe, approximated_z_tp1, t, prompt_embeds_in, added_cond_kwargs_in,down_block_res_samples=down_block_res_samples,mid_block_res_samples=mid_block_res_samples)
    ...
    if pipe.cfg.perform_noise_correction:
        noise_pred = unet_pass(pipe, approximated_z_tp1, t, prompt_embeds, added_cond_kwargs,down_block_res_samples=down_block_res_samples,mid_block_res_samples=mid_block_res_samples)
...
@torch.no_grad()
def unet_pass(pipe, z_t, t, prompt_embeds, added_cond_kwargs, down_block_res_samples=None, mid_block_res_samples=None):
    latent_model_input = torch.cat([z_t] * 2) if pipe.do_classifier_free_guidance else z_t
    latent_model_input = pipe.scheduler.scale_model_input(latent_model_input, t)
    return pipe.unet(
        latent_model_input,
        t,
        encoder_hidden_states=prompt_embeds[:,0:77],
        timestep_cond=None,
...
class DGPSTPipeline(
    ...
    def corrm(self, fea, fea0):
        ...
        fea0 = fea0 - fea0.mean(dim=1,keepdim=True)
        # fea0 = fea0.view(b,c,-1)
        fea0_norm = (torch.norm(fea0, 2, 1, keepdim=True)+sys.float_info.epsilon)
        fea0 = fea0 / fea0_norm
        fea0 = fea0.permute(0,2,1).contiguous()
        corr = F.softmax(torch.matmul(fea0, fea) / 0.01, dim=-1)
        return corr
    def warp(self, fea, corr, target=None):
        b,c,h,w = fea.size()
        l = h * w
        B,H,W = corr.size()
        if H != l:
    ...
    def prepare_latents2(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None, image_whole=None):
        ...
        image = image.to(device=device, dtype=dtype)
        if image_whole!=None:
            image_whole = image_whole.to(device=device, dtype=dtype)
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                if image_whole!=None:
                    init_image_whole = [
                        retrieve_latents(self.vae.encode(image_whole[i : i + 1]), generator=generator[i])
                        for i in range(batch_size)
                    ]
                    init_image_whole = torch.cat(init_image_whole, dim=0) 
            else:
                init_latents = retrieve_latents(self.vae.encode(image), generator=generator)
                if image_whole!=None:
                    init_image_whole = retrieve_latents(self.vae.encode(image_whole), generator=generator)

            init_latents = self.vae.config.scaling_factor * init_latents
            if image_whole!=None:
                init_image_whole = self.vae.config.scaling_factor * init_image_whole
        if batch_size > init_latents.shape[0] and batch_size % init_latents.shape[0] == 0:
            ...
            if image_whole!=None:
                init_image_whole = torch.cat([init_image_whole] * additional_image_per_prompt, dim=0)
        ...
        if image_whole!=None:
            # corr = self.corrm(init_image_whole,init_latents)
            # init_latents = self.warp(init_image_whole,corr)
            if timestep-1>=0:
                image_whole_latents = self.scheduler.add_noise(init_image_whole, noise, timestep)
            else:
                image_whole_latents = init_image_whole
            return latents, noise, image_whole_latents
        latents = latents
    ...
    def extract_corr(
        ...
        clip_skip: Optional[int] = None,
        train_loss: Optional[bool] = False,
        image_sd: Optional[PipelineImageInput] = None,
        ...
        with torch.no_grad():
            ...
            # 4. Prepare image
            image_sd = self.image_processor.preprocess(image_sd)
            if ref_image!=None:
            ...
            if train_loss:
                ...
                timesteps, num_inference_steps = self.get_timesteps(num_inference_steps, 0.999, device)
                id = 0
                latent_timestep = timesteps[id:id+1]
            else:
            ...
            latents = self.vae.encode(image_sd.cuda()).latent_dist.sample() * self.vae.config.scaling_factor
            t = torch.tensor(1, dtype=torch.long, device=device)
            noise = torch.randn_like(latents).to(device)
            latents = self.scheduler.add_noise(latents, noise, t).to(dtype=torch.float32)
        ...
        # control_model_input = torch.cat((control_model_input,control_model_input),dim=1)
        ...
        feat = self.unet.forward_feature(
            latent_model_input,
            t,
            up_ft_indices,
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            num_channels_latents = self.unet.config.in_channels
            if train_loss:
                latents, noise, _ = self.prepare_latents2(
                    image_sd, latent_timestep, batch_size, num_images_per_prompt, prompt_embeds.dtype, device, generator,
                )
            else:    
                if latents==None:
                    latents, noise, latents_ref = self.prepare_latents2(
                        image_sd, latent_timestep, batch_size, num_images_per_prompt, prompt_embeds.dtype, device, generator, image_whole=ref_image,
                    )
                    latents = latents_ref
                if mask!=None:
                    latents, noise, image_latents = self.prepare_latents2(
                        image_sd, latent_timestep, batch_size, num_images_per_prompt, prompt_embeds.dtype, device, generator, image_whole=None,
                    )                        
        ...
        if train_loss == True:
            ...
            loss_dict = {}
            loss = self.get_loss(noise_pred.to(dtype=torch.float32), noise.to(dtype=torch.float32), mean=False).mean(dim=[1, 2, 3])
            log_prefix = 'train' 
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guess_mode and do_classifier_free_guidance:
                    ...
                    control_model_input = latent_model_input
                    controlnet_prompt_embeds = prompt_embeds
                # control_model_input = torch.cat((control_model_input,latents_ref_cont),dim=1)
                if isinstance(controlnet_keep[i], list):
                    cond_scale = [c * s for c, s in zip(controlnet_conditioning_scale, controlnet_keep[i])]
                else:
                    controlnet_cond_scale = controlnet_conditioning_scale
                ...
                if guess_mode and do_classifier_free_guidance:
                    ...
                    # down_block_res_samples = [torch.cat([torch.zeros_like(d), d]) for d in down_block_res_samples]
                    # mid_block_res_sample = torch.cat([torch.zeros_like(mid_block_res_sample), mid_block_res_sample])
                    down_block_res_samples = [torch.cat([d, d]) for d in down_block_res_samples]
                    mid_block_res_sample = torch.cat([mid_block_res_sample, mid_block_res_sample])
                ...
                # if t==1:
                #     latents = self.scheduler.step(noise_pred, t, latents, direct0=1, **extra_step_kwargs, return_dict=False)[0]
                ...
                if mask != None:
                    ...
                    init_mask = F.interpolate(init_mask,scale_factor=0.125,mode="bilinear")
