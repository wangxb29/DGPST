...
class TmuxLauncher():
    ...
    def commands(self):
        ...
        return ["python -m torch.distributed.launch --nproc_per_node=1 --master_port=29510 train.py " + str(opt) for opt in opts]
    
    def test_commands(self):
        opts = self.test_options()
        return ["python -m torch.distributed.launch --nproc_per_node=1 --master_port='29518' test.py " + str(opt) for opt in opts]
    ...
    def refine_command(self, command, resume_iter, continue_train, gpu_id=None):
        ...
        #command = "{} {}".format(command, resume_iter)
